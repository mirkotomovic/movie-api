  ...
  return (
    <section className="person-container">
      <article className="person-basic">
        <img src={person.image ? person.image.original : "#"}
          alt={person.name}
          height="300px"
          width="210px" />
        <ul className="about-person">
          ...
          <li><b>Country: </b>{person.country?.name}</li>
      ...
      {castcredits.length !== 0 ?
        <article className="cast-person">
          ...
          {castcredits.map(credit =>
            <Link key={credit._embedded.show.id} to={"/show/" + credit._embedded.show.id}>
              <ShowCard props={credit._embedded.show} />
            </Link>
          )}
        </article> : <article className="cast-person"><h3>No cast credit</h3></article>
      }
      {crewcredits.length !== 0 ?
        <article className="crew-person">
          ...
          {crewcredits.map(credit =>
            <Link key={credit._embedded.show.id} to={"/show/" + credit._embedded.show.id}>
              <ShowCard key={credit._embedded.show.id} props={credit._embedded.show} />
            </Link>
          )}
        </article> : <article className="crew-person"><h3>No crew credit</h3></article>
      }
    </section>
  )
}
